...
{
	float vertexData[] = {
		// vertex attributes for a quad that fills the entire screeen in Normalized Device coordinates
		// positions  texCoords
		-1.0f, 1.0f,  0.0f,  1.0f,
		-1.0f, -1.0f, 0.0f,  0.0f,
		1.0f,  -1.0f, 1.0f,  0.0f,
...
void PostProcessor::BindVertices()
{
	...
	glVertexAttribPointer(
		m_postShader->GetAttrVertices(),	// The attribute we want to configure
		2,									// size (2 components)
		GL_FLOAT,							// type
		GL_FALSE,							// normalized?
	...
	glVertexAttribPointer(
		m_postShader->GetAttrTexCoords(),	// The attribute we want to configure
		2,									// size (2 components)
		GL_FLOAT,							// type
		GL_FALSE,							// normalized?
		4 * sizeof(float),					// stride floats per vertex definition
